...
static const char *apcc_clk_names[APCC_NUM_CLOCKS] = {
	"cpu0-mux-clk", "cpu4-mux-clk", "cci-mux-clk",
	"cpu0-pll-clk", "cpu4-pll-clk", "cci-pll-clk",
};

static const struct pll_vco apcc_msm8953_pll_vco[] = {
	VCO(0, 652800000, 2208000000),
};

static const struct pll_vco apcc_sdm632_pll_vco[] = {
	VCO(0, 614400000, 2016000000), /* Pwr cluster PLL */
	VCO(0, 633600000, 2016000000), /* Perf cluster PLL */
	VCO(2, 500000000, 1000000000), /* CCI PLL */
};

static const u8 apcc_alpha_pll_regs[PLL_OFF_MAX_REGS] = {
	[PLL_OFF_L_VAL]		= 0x08,
...
static const struct alpha_pll_config msm8953_pll_cfg = {
...
static const struct alpha_pll_config sdm632_cci_pll_cfg = {
...
static int apcc_msm8953_register_pll(struct device *dev, int pll_idx, int num_plls)
{
	...
	apll->vco_table = &apcc_msm8953_pll_vco[pll_idx];
	apll->num_vco = 1;
	apll->offset = 0;
	apll->regs = apcc_alpha_pll_regs;
	pll_config = &msm8953_pll_cfg;
	data->hws[clk_idx] = &apll->clkr.hw;

	if (num_plls > ARRAY_SIZE(apcc_msm8953_pll_vco)) {
		apll->vco_table = &apcc_sdm632_pll_vco[pll_idx];
		apll->num_vco =  ARRAY_SIZE(apcc_sdm632_pll_vco);
	}

	if (clk_idx == APCC_CCI_PLL) {
		apll->regs = clk_alpha_pll_regs[CLK_ALPHA_PLL_TYPE_DEFAULT];
		pll_config = &sdm632_cci_pll_cfg;
	}
...
static int apcc_msm8953_register_mux(struct device *dev, int clk_idx, int num_plls)
{
	...
	// CCI on MSM8953 probably shouldn't have this */
