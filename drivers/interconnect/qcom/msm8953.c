...
enum {
	MSM8953_MASTER_CCI = 1,
	MSM8953_CCI_BIMC_SLV,
	MSM8953_MASTER_AMPSS_M0,
...
struct cci_provider {
	...
	struct clk *l2_clk, *l2_pll;
	bool clk_inited;
	struct icc_onecell_data data;
};

static int msm8953_cci_config_clks(struct device *dev,
		struct opp_table *opp_table,
		struct dev_pm_opp *opp,
		void *data,
		bool scaling_down)
{
	struct cci_provider *cci = dev_get_drvdata(dev);
	unsigned long rate;
	int ret;

	if (likely(cci->clk_inited))
		return 0;

	ret = clk_rate_exclusive_get(cci->l2_pll);
	if (ret)
		return ret;

	rate = clk_get_rate(cci->l2_pll);
	if (rate) {
		clk_set_rate(cci->l2_clk, (rate * 2) / 5);
		cci->clk_inited = true;
	}

	clk_rate_exclusive_put(cci->l2_pll);

	return 0;
}

static int msm8953_cci_set(struct icc_node *src, struct icc_node *dst)
{
	return dev_pm_opp_set_rate(src->provider->dev,
			max(src->avg_bw, src->peak_bw) >> 3);
}

static int msm8953_cci_get_bw(struct icc_node *node, u32 *avg, u32 *peak)
{
	struct cci_provider *cci;

	cci = container_of(node->provider->data, struct cci_provider, data);
	*avg = *peak = clk_get_rate(cci->l2_clk) << 3;
	return 0;
}
...
static struct {
...
} msm8953_icc_nodes[] = {
	{
		.name = "mas_l2",
		.index = MAS_CCI,
		.id = MSM8953_MASTER_CCI,
		.link = MSM8953_CCI_BIMC_SLV,
	},
	{
		.name = "slv_l2_bimc",
		.index = SLV_CCI_BIMC,
		.id = MSM8953_CCI_BIMC_SLV,
		//.link = MSM8953_MASTER_AMPSS_M0,
	}
};

static int msm8953_cci_probe(struct platform_device *pdev)
{
	...
	struct cci_provider *cci;
	struct icc_onecell_data *data;
	const int num_nodes = ARRAY_SIZE(msm8953_icc_nodes);
	int ret, i;

	cci = devm_kzalloc(dev,
			offsetof (struct cci_provider, data.nodes[num_nodes]),
			GFP_KERNEL);
	if (!cci)
		return -ENOMEM;

	dev_set_drvdata(dev, cci);

	ret = PTR_ERR_OR_ZERO(cci->l2_clk = devm_clk_get(dev, "cci"));
	if (ret)
		return dev_err_probe(dev, ret, "failed to get cci clock\n");

	ret = PTR_ERR_OR_ZERO(cci->l2_pll = devm_clk_get(dev, "cci-pll"));
	if (ret)
		return dev_err_probe(dev, ret, "failed to get cci-pll clock\n");

	ret = dev_pm_domain_attach(dev, true);
	if (ret)
		return dev_err_probe(dev, ret, "failed attach PM domain\n");

	config.genpd_names = (const char *[]) { "cpupd", NULL },
	config.clk_names = (const char *[]) { "cci", NULL };
	config.config_clks = msm8953_cci_config_clks;
	...
	provider = &cci->provider;
	INIT_LIST_HEAD(&provider->nodes);
	provider->dev = dev;
	provider->aggregate = icc_std_aggregate;
	provider->get_bw = msm8953_cci_get_bw;
	provider->set = msm8953_cci_set;
	...
	for (i = 0; i < num_nodes; i++) {
		struct icc_node *node = icc_node_create(msm8953_icc_nodes[i].id);
		ret = PTR_ERR_OR_ZERO(node);
		if (ret)
			return dev_err_probe(dev, ret, "failed to create icc node\n");

		node->name = msm8953_icc_nodes[i].name;
		data->nodes[msm8953_icc_nodes[i].index] = node;
		icc_node_add(node, provider);
		if (msm8953_icc_nodes[i].link)
			icc_link_create(node, msm8953_icc_nodes[i].link);
	}

	cci->data.num_nodes = num_nodes;
	return 0;
}

void msm8953_icc_sync(struct device* dev) {
	if (of_device_is_compatible(dev->of_node, "qcom,msm8953-ccic"))
		icc_sync_state(dev);

	/* Rest is not ready =( */
}

int msm8953_icc_probe(struct platform_device *pdev)
{
	if (of_device_is_compatible(pdev->dev.of_node, "qcom,msm8953-ccic"))
		return msm8953_cci_probe(pdev);

	return qnoc_probe(pdev);
}

int msm8953_icc_remove(struct platform_device *pdev)
{
	if (of_device_is_compatible(pdev->dev.of_node, "qcom,msm8953-ccic"))
		return 0;

	return qnoc_remove(pdev);
}

static const struct of_device_id msm8953_icc_of_match[] = {
	{ .compatible = "qcom,msm8953-ccic" },
...
static struct platform_driver msm8953_noc_driver = {
	.probe = msm8953_icc_probe,
	.remove = msm8953_icc_remove,
	.driver = {
		.name = "qnoc-msm8953",
		.of_match_table = msm8953_icc_of_match,
...
MODULE_DEVICE_TABLE(of, msm8953_icc_of_match);
MODULE_DESCRIPTION("Qualcomm MSM8953 interconnect driver");
